"""Single-agent runtime with MySQL history and asynchronous summarization."""
...
from app.providers.audit import capture_model_call, serialize_ai_message
...
class AgentRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        self.system_prompt = build_system_prompt(self.tools, self.skills)
    ...
    def begin_chat(
        ...
        *,
        ...
        selected_context_window = validate_context_window_tokens(
            ...
            with capture_model_call(selected_model_id) as capture:
                try:
                    ...
                    context_plan = plan_context(
                        window_tokens=round_state["contextWindowTokens"],
                        ...
                        history_groups=_history_message_groups(
                            uncovered_messages
                        ),
                        current_message=HumanMessage(content=message),
                    )
                    model_messages = list(context_plan.messages)
                    ...
                    if (
                        ...
                        retry_validation_message = SystemMessage(
                            content=(
                                "# 输出校验失败\n\n"
        ...
        with _runtime_lock:
            if _default_runtime is None:
                ...
                meeting_drafts = MeetingRoomDraftStore(
                    meeting_gateway,
                    booked_by=sandbox_http.settings.user_name,
                ...
                skills = [
                    ...
                    create_leave_skill(MockSandboxLeaveClient(sandbox_http)),
                ]
...
def _history_message_groups(
    messages: list[dict[str, Any]],
) -> list[list[BaseMessage]]:
    """Group history by complete round so trimming never splits a turn."""
    grouped: dict[int, list[dict[str, Any]]] = {}
    for message in messages:
        grouped.setdefault(int(message["round"]), []).append(message)
    return [
        _to_langchain_messages(round_messages)
        for round_messages in grouped.values()
    ]
