...
`people-directory` Skill 提供一个可组合的 `find_person` Tool。Tool 使用固定优先级中的首个
非空主要条件调用一次外部人员查询接口 `/api/eop-olk/api/v2/addressbook/search`，访问 18080
测试沙箱，并把宽匹配候选映射为：
...
Tool 支持一次传入工号、完整手机号、姓名、手机号尾号和附加部门条件。工号、完整手机号、姓名
或手机尾号至少需要一个，部门不能单独使用。远程候选依次经过本地 `filter_by_*`；工号和完整
手机号精确匹配，姓名按字段包含匹配，手机尾号使用 `endswith`，部门精确匹配，最终只返回同时
满足全部条件的 `people`。Tool 不判断用户说法是否正确；待核对的说法由 Agent 与返回人员字段
进行有限度的辅助比较。完整协议见
...
- `people-directory` Skill：提供一个可组合的 `find_person` Tool。
  - Tool 接受工号、完整手机号、姓名、手机尾号和附加部门条件；按固定优先级选择一个主要条件
    调用一次外部接口，再用全部非空条件过滤同一候选列表。
  - 部门不能单独查询；姓名按字段包含匹配，数字类宽召回候选会在本地按真实工号、完整手机号或
    手机尾号再次验证。空结果只表示没有共同匹配，显式空值和非法手机号文本在请求前拒绝。
